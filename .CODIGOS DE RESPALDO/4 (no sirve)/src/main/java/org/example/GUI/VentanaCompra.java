...
public class VentanaCompra extends JFrame {
    ...
    public VentanaCompra(String usuario) {
        ...
        this.conexion = ConexionNeo4j.getInstance();
    ...
    private List<String> obtenerProductosDisponibles() {
        try {
            System.out.println("=== Debug de Conexión y Productos ===");
            System.out.println("Usuario actual: " + usuario);
            
            var session = conexion.getSession();
            System.out.println("Sesión creada exitosamente");
            
            // Verificar la conexión
            var testResult = session.run("RETURN 1 as test");
            if (testResult.hasNext()) {
                System.out.println("Conexión a Neo4j exitosa");
            }

            // Verificar todos los nodos
            var allNodesResult = session.run("MATCH (n) RETURN labels(n) as labels, count(n) as count");
            while (allNodesResult.hasNext()) {
                var record = allNodesResult.next();
                System.out.println("Nodos tipo " + record.get("labels") + ": " + record.get("count").asInt());
            }

            // Obtener productos
            var result = session.run("MATCH (p:Producto) RETURN p.nombre as nombre");
