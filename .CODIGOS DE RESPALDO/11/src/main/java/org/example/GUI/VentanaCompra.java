...
public class VentanaCompra extends JFrame {
    ...
    private boolean realizarCompra(String producto) {
        try {
            // Verificamos stock y existencia de la relación en una sola consulta
            var result = conexion.getSession().run(
                "MATCH (u:Usuario {nombre: $usuario}), (p:Producto {nombre: $producto}) " +
                "WHERE p.stock > 0 " +
                "OPTIONAL MATCH (u)-[r:COMPRÓ]->(p) " +
                "WITH u, p, r, p.stock as stockActual " +
                "WHERE stockActual > 0 " +
                "SET p.stock = p.stock - 1 " +
                "FOREACH (ignoreMe IN CASE WHEN r IS NULL THEN [1] ELSE [] END | " +
                "   CREATE (u)-[:COMPRÓ]->(p)) " +
                "RETURN stockActual > 0 as compraRealizada",
                org.neo4j.driver.Values.parameters("usuario", usuario, "producto", producto)
            );
            
            boolean compraExitosa = result.single().get("compraRealizada").asBoolean();
